...
public class EndPointRestSaccheInScadenza implements EndPointSaccheInScadenzaCCS{
	...
	public void aggiungiSaccaInScadenza(Sacca[] listaSaccheInScadenza) throws EntityAlreadyExistsException{
		...
		for(Sacca s : new ArrayList<>(Arrays.asList(listaSaccheInScadenza))) {
			mm.createSacca(s);
			this.broadcastToCTT(s);
		}
	}

	private void broadcastToCTT(Sacca s) {
		List<CTT> centers = mm.getListaCTT();
		Client client = ClientBuilder.newClient();

		String ip = CTTStaticIpMap.indirizziCTT.get("CTT001");
		WebTarget gestioneSaccheInscadenza = client.target(ip).path("/notifica/alertScadenza");
		gestioneSaccheInscadenza.request().post(Entity.json(s));

		/*
		for (CTT ctt : centers) {
			String ip = CTTStaticIpMap.indirizziCTT.get(ctt.getDenominazione().getCttname());
			WebTarget gestioneSaccheInscadenza = client.target(ip).path("/notifica/alertScadenza");
			gestioneSaccheInscadenza.request().post(Entity.json(s));
		}
